...
plt.xticks([],fontsize=20); plt.yticks([1,2,3,4],fontsize=20);
#plt.yscale('log')
plt.xlim(4,126)
plt.ylim(0.5,4.0)
...
plt.xticks([],fontsize=20); plt.yticks([0,1,2],fontsize=20);
#plt.ylabel(r'$\frac{dW_x}{dx}$ [m$_e$c$\omega_0$]',fontdict=font)
plt.ylabel(r'$\frac{dW_x}{dx}$',fontdict=font)
plt.xlim(4,126)
plt.ylim(-1.0,2.4)
...
y1 = -(fieldey*py/px)[index,:]
y2 = np.zeros_like(y1)
ax.fill_between(xx[index,:],y1,y2, where=y1>=y2, facecolor='green',alpha=0.3,interpolate=True)
ax.fill_between(xx[index,:],y1,y2, where=y1<=y2, facecolor='red',alpha=0.3,interpolate=True)
plt.scatter(xx[index,:], y1, c=gg[index,:], norm=norm_x, s=6, cmap='autumn', edgecolors='None')
plt.xticks([],fontsize=20); plt.yticks([0,0,1,2],fontsize=20);
...
y1 = -(fieldey*py/px)[index,:]-(fieldex)[index,:]
y2 = np.zeros_like(y1)
ax.fill_between(xx[index,:],y1,y2, where=y1>=y2, facecolor='green',alpha=0.3,interpolate=True)
ax.fill_between(xx[index,:],y1,y2, where=y1<=y2, facecolor='red',alpha=0.3,interpolate=True)
plt.scatter(xx[index,:], y1, c=gg[index,:], norm=norm_x, s=6, cmap='autumn', edgecolors='None')
...
plt.subplots_adjust(left=None, bottom=None, right=None, top=None,wspace=None,hspace=0.031)


fig = plt.gcf()
fig.set_size_inches(11.0, 11)
fig.savefig('./figure_wrap_up/'+'trajectory_power_x.png',format='png',dpi=160)
plt.close("all")
